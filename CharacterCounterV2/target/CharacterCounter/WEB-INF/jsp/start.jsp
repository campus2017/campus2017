...
<form action="/CharacterCounter/statistics" method="post" accept-charset="UTF-8">
    <div id="file2" class="change fixed" style="width:400px;display:none;">
    <textarea name="text" id="text" ng-model="saveText.text" cols="30" rows="4"
              style="border:1px solid grey;overflow-y: scroll;float:left;margin-left:20px;resize:none;"
              placeholder="请再此输入文本内容"></textarea>

        <input type="submit" value="统计" style="height:30px;margin:0 0 0 20px ;padding:0 26px;float:left;
        background-color: deepskyblue"/>
        <div class="button  button-small "
             style="height:30px;margin:8px 0 0  20px ;padding:0 17.6px;float:left;background: orange;"
             ng-click="clear()">
            <b>清空内容</b></div>
    </div>
</form>
...
<table width="300" border="2" cellpadding="10" cellspacing="10" style="margin-left:20px;">
    ...
    <tr>
        ...
        <td style="border:1px solid #000;text-align: center;">${result.data.english}</td>
    </tr>
    <tr>
        <td style="border:1px solid #000;text-align: center;padding:6px; ">数字</td>
        <td style="border:1px solid #000;text-align: center;">${result.data.number}</td>
    </tr>
    <tr>
        <td style="border:1px solid #000;text-align: center; padding:6px;">中文汉字</td>
        <td style="border:1px solid #000;text-align: center;">${result.data.chinese}</td>
    </tr>
    <tr>
        <td style="border:1px solid #000;text-align: center;padding:6px; ">中英文标点符号</td>
        <td style="border:1px solid #000;text-align: center;padding:6px;">${result.data.punctuation}</td>
    </tr>
    </tbody>
</table>
<br/>
...
<table width="300" border="2" cellpadding="10" cellspacing="10" style="margin-left:20px;">
    ...
    <tr>
        <td style="border:1px solid #000;text-align: center; height:21px;padding:0 6px;">${result.mostList[0].name}</td>
        <td style="border:1px solid #000;text-align: center;">${result.mostList[0].num}</td>
    </tr>
    <tr>
        <td style="border:1px solid #000;text-align: center; height:21px;padding:0 6px;">${result.mostList[1].name}</td>
        <td style="border:1px solid #000;text-align: center;">${result.mostList[1].num}</td>
    </tr>
    <tr>
        <td style="border:1px solid #000;text-align: center; height:21px;padding:0 6px;">${result.mostList[2].name}</td>
        <td style="border:1px solid #000;text-align: center;">${result.mostList[2].num}</td>
    </tr>
</table>
