...
(function (ionic) {
  ...
  ionic.DelegateService = function(methodNames) {
    ...
    return ['$log', function($log) {
       ...
       * and call them on the given the contr instance matching given
       * handle.
       * The reason we don't just let $getByHandle return the contr instance
       * itself is that the contr instance might not exist yet.
       *
       * We want people to be able to do
       * `var instance = $ionicScrollDelegate.$getByHandle('foo')` on contr
       * instantiation, but on contr instantiation a child directive
       * may not have been compiled yet!
       *
       * So this is our way of solving this problem: we create an object
       * that will only try to fetch the contr with given handle
        ...
        return function caller() {
          ...
          if (!foundInstancesCount && handle) {
            return $log.warn(
              ...
              'contr, then your element may not be compiled yet. Put a $timeout ' +
...
(function(window, document, ionic) {
   ...
   * .contr('PlatformCtrl', function($scope) {
   *
